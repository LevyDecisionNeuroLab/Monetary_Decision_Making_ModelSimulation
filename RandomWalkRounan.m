...
tic
h = waitbar(0,'Simulating RandomWalk...');

probs = [.5]';
ambs = [.24 .5 .74 1.0]';

%vals = [5,6,7,8,10,12,14,16,19,23,27,31,37,44,52,61,73,86,101,120];
vals = [4,5,6,6,7,7,8,9,10,11,13,14,15,19,24,29,37,51,95,120];
...
while mean(sortvals_rank(1,valperms-5:valperms))<.8 && n<61 %Up
    ...
    for vals_index = 1:size(v1,2);
                ...
                for sims = 1:simnumbs
                    ...
                    while sum(sum(Cresults==1)) > 90 || sum(sum(Cresults==0)) > 90
                        ...
                        alpha(alpha<0) = 0;
                        beta = -1.5 + 3.*rand(1);          %Gains Normal distribution of measured Ambiguity Preferences
...
%                     r2N = info.r2;
    ...
    % 
    % 
                 ralpha(vals_index) = r(1,1);
                 rbeta(vals_index) = r(2,2);
    ...
    %             disp(ambs_index+(probs_index-1)*length(ambs))
                waitbar(vals_index/(size(v1,2)),h,['Simulating RandomWalk... Loop: ' num2str(n)]);
    end

    rsqprd = (ralpha.^2).*(rbeta.^2);
    
    topval = max(v1);
    expense_constant = .001;
    expense = (120-topval).*expense_constant;
    
    costfunc = reshape(rsqprd,[size(rsqprd,2),1])+expense';
    
    tempX = [costfunc reshape(rsqprd,[size(rsqprd,2),1]) reshape(ralpha,[size(rsqprd,2),1]) reshape(rbeta,[size(rsqprd,2),1]) v1'];
    sortvals_rank = sortrows(tempX,1)';
    sortvals = sortvals_rank(5:end,:);
...
converge = reshape(mean(sortvalsrank_multi(1:2,(1+size(tempX,1)/2):size(tempX,1),:),2),[2,size(sortvalsrank_multi,3)])';
alphabeta = reshape(mean(sortvalsrank_multi(3:4,(1+size(tempX,1)/2):size(tempX,1),:),2),[2,size(sortvalsrank_multi,3)])';
drift = reshape(mean(sort(sortvalsrank_multi(5:end,:,:)),2),[20,size(sortvalsrank_multi,3)]);

figure;
plot(1:length(converge),converge(:,1))
hold on
plot(1:length(converge),converge(:,2),'r')
legend('costfunction','sqrprd');

figure;
plot(1:size(sortvalsrank_multi,3),drift);

figure;
plot(1:length(alphabeta),alphabeta(:,1));
title('Alpha');
figure
plot(1:length(alphabeta),alphabeta(:,2));
title('Beta');

close(h) 
clearvars -except MultiSubChoices Cresults rsqprd ralpha rbeta r v1 vals ambs probs splitvals sortvals sortvals_rank monvals n sortvalsrank_multi converge drift alphabeta
