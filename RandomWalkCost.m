...
while nanmean(sortvals_rank(1,valperms-10:valperms))<.8 && n<3 %Up
            ...
            for ambs_index = 1:length(ambs)
                ...
                for sims = 1:simnumbs
                    ...
                    while sum(sum(Cresults==1)) > 35 || sum(sum(Cresults==0)) > 35
                        ...
                        for valindex = 1:20
                            for riskindex = 1
                                Cresults(riskindex,valindex) = nanmean(Choices(V==v(valindex) & P==pr(riskindex) & A==0));
                            end
                            for ambindex = 2
                                Cresults(ambindex,valindex) = nanmean(Choices(and(V==v(valindex),A==a(ambindex-1))));
                            end
                        end
                    end
    ...
    costfunc = reshape(rsqprd,[size(rsqprd,3),1])+expense';
    
    tempX = [costfunc reshape(rsqprd,[size(rsqprd,3),1]) reshape(ralpha,[size(rsqprd,3),1]) reshape(rbeta,[size(rsqprd,3),1]) v1'];
...
converge = reshape(nanmean(sortvalsrank_multi(1:2,(1+size(tempX,1)/2):size(tempX,1),:),2),[2,size(sortvalsrank_multi,3)])';
alphabeta = reshape(nanmean(sortvalsrank_multi(3:4,(1+size(tempX,1)/2):size(tempX,1),:),2),[2,size(sortvalsrank_multi,3)])';
drift = reshape(nanmean(sort(sortvalsrank_multi(5:end,:,:)),2),[20,size(sortvalsrank_multi,3)]);
...
close(h) 
clearvars -except rsqprd ralpha rbeta r v1 vals ambs probs splitvals sortvals sortvals_rank monvals n sortvalsrank_multi converge drift alphabeta
