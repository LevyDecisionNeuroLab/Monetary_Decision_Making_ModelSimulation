...
tic
h = waitbar(0,'Simulating BestSimple Data...');
...
for probs_index = 1:length(probs)
    for ambs_index = 1:length(ambs)
        ...
        ralpha(probs_index,ambs_index) = r(1,1);
        rbeta(probs_index,ambs_index) = r(2,2);
        ...
        waitbar((ambs_index+(probs_index-1)*length(ambs))/(length(ambs)*length(probs)),h);
    end
end
