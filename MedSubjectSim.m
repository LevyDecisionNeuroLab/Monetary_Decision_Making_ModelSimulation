...
tic
h = waitbar(0,'Simulating MedMon Data...');
...
for sims = 1:simnumbs
    ...
    while sum(sum(Cresults==1)) > 34 || sum(sum(Cresults==0)) > 34 || sum(Cresults(:,1)) < 4
        ...
        for valindex = 1:5
            for riskindex = 1:5
                Cresults(riskindex,valindex) = mean(Choices(V==v2(valindex) & P==p(riskindex) & A==0));
            end
            for ambindex = 6:8
                Cresults(ambindex,valindex) = mean(Choices(and(V==v2(valindex),A==a(ambindex-5))));
            end
        end
    end
    
    
    MultiSubChoices(:,:,sims) = [Vfit,P,A,Choices];
    MultiSubResults(:,:,sims) = Cresults;
    MultiParams(sims,:) = [alpha,beta];
    MultiNoise(sims) = noise;
    MultiFOSD(sims) = 32-sum(Cresults(:,1))/.25;
    MultiValues(sims,:) = v2;
...
for s = 1:length(Subindex)
    ...
    waitbar(s/length(Subindex),h,'Fitting MedMon Data...')
